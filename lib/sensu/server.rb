  ...
  class Server
    ...
    def setup_redis
      @logger.debug('connecting to redis', {
        :settings => @settings.redis.to_hash
      })
    ...
    def process_result(result)
      ...
      @redis.get('client:' + result[:client]).callback do |client_json|
        unless client_json.nil?
          ...
          check = case
          when @settings.checks.key?(result[:check][:name])
            @settings.checks[result[:check][:name]].to_hash.merge(result[:check])
          else
            result[:check]
          end
          event = {
            ...
            @timers << EM::PeriodicTimer.new(interval) do
              unless @rabbitmq.reconnecting?
                ...
                check[:subscribers].each do |exchange|
                  @amq.fanout(exchange).publish(payload.to_json)
                end
              end
            end
          end
